...
export function getPath(obj: object | any[], path: string = ""): any {
  ...
  return path
    .split(".")
    .reduce((out, key) => (out ? (out as any)[key] : undefined), obj);
}
...
export async function getSecurityExtensions(
  baseUrl: string
): Promise<SMART.OAuthSecurityExtensions> {
  const url = String(baseUrl || "").replace(/\/*$/, "/") + "metadata";
  let metadata;
  try {
    metadata = await request(url);
  } catch (ex) {
    metadata = {};
  }
  const nsUri =
...
export async function buildAuthorizeUrl(
  ...
  if (!serverUrl) {
    debug(
      "No serverUrl found. It must be specified as query.iss or " +
...
export async function buildTokenRequest(
  ...
  if (cached.clientSecret) {
    requestOptions.headers.Authorization =
      "Basic " +
      Buffer.from(cached.clientId + ":" + cached.clientSecret).toString(
        "base64"
      );
    debug(
...
export function getSessionStorage(
  req: SMART.HttpRequestWithSession
): SMART.Storage {
  return {
    /**
